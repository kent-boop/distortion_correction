...
class EncoderNet(nn.Module):
    def __init__(self, layers):
        ...
        # self.en_layer6 = self.make_encoder_layer(resEncoderBlock, 512, 1024, layers[5], stride=2)#新增一层
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.bias.data.zero_()
            elif isinstance(m, (nn.Conv2d, nn.ConvTranspose2d)):  # 覆盖转置卷积
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
                m.weight.data.normal_(0, math.sqrt(2. / n))
    ...
    def forward(self, x):
        ...
        x = self.en_layer4(x)
        x = self.en_layer5(x)
        # x = self.en_layer6(x) #新增的
        # 在EncoderNet的forward方法末尾添加
        # print("Encoder输出尺寸:", x.shape)  # 如torch.Size([b, 512, 8, 8])
...
class DecoderNet(nn.Module):
    def __init__(self, layers):
        ...
        #新增一层
        # self.de_layer6 = self.make_decoder_layer(resDecoderBlock, 1024, 512, layers[5], stride=2)
    ...
    def forward(self, x):
        # # 在DecoderNet的forward方法开头添加
        # print("Decoder输入尺寸:", x.shape)  # 需与Encoder输出相同
        # x = self.de_layer6(x)#新增的
        x = self.de_layer5(x)
        x = self.de_layer4(x)
        x = self.de_layer3(x)
        x = self.de_layer2(x)
