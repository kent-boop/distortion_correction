...
parser.add_argument('--patience', type=int, default=4, help='早停法等待轮数')
...
def train(train_loader,val_loader,epochs,optimizer,criterion,criterion_clas):
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            if avg_loss < best_val_loss:
                ...
                no_improve_counter = 0
                torch.save(model_en.state_dict(), 'best_model_dropout_en.pkl')
                torch.save(model_de.state_dict(), 'best_model_dropout_de.pkl')
                torch.save(model_class.state_dict(), 'best_model_dropout_class.pkl')
                print("发现新最佳模型，已保存!")
            else:
        ...
        # 保存模型
        torch.save(model_en.state_dict(), f'model_dropout_en_{epoch + 1}.pkl')
        torch.save(model_de.state_dict(), f'model_dropout_de_{epoch + 1}.pkl')
        torch.save(model_class.state_dict(), f'model_dropout_class_{epoch + 1}.pkl')
    ...
    torch.save(model_en.state_dict(), 'model_dropout_en_last.pkl')
    torch.save(model_de.state_dict(), 'model_dropout_de_last.pkl')
    torch.save(model_class.state_dict(), 'model_dropout_class_last.pkl')
    writer.close()
...
pretrained_en = torch.load('model_distill_en_4.pkl',weights_only=True,map_location=device)
model_en.load_state_dict(pretrained_en,strict=False)  # 关键参数
model_en = model_en.to(device)

pretrained_de = torch.load('model_distill_de_4.pkl',weights_only=True,map_location=device)
model_de.load_state_dict(pretrained_de,strict=False)
model_de = model_de.to(device)

pretrained_class = torch.load('model_distill_class_4.pkl',weights_only=True,map_location=device)
model_class.load_state_dict(pretrained_class,strict=False)
model_class = model_class.to(device)

# # 加载后检查不匹配的键
# en_missing, en_unexpected = model_en.load_state_dict(pretrained_en, strict=False)
# de_missing, de_unexpected = model_de.load_state_dict(pretrained_de, strict=False)
# class_missing, class_unexpected = model_class.load_state_dict(pretrained_class, strict=False)
#
# print("Encoder 缺失的键:", en_missing)
# print("Encoder 意外的键:", en_unexpected)
# print("Decoder 缺失的键:", de_missing)
# print("Decoder 意外的键:", de_unexpected)
# print("class 缺失的键:", class_missing)
# print("class 意外的键:", class_unexpected)
