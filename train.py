...
from model import EncoderNet, DecoderNet, ClassNet, EPELoss
    ...
    for epoch in range(epochs):
        ...
        for i, (disimgs, disx, disy, labels) in enumerate(train_loader):
            ...
            if step % 100 == 0:
                ...
                step_interval = current_time - (start_time if step == 100 else previous_step_time)
                previous_step_time = current_time

                print(f"Epoch [{epoch + 1}], Step [{step}], Loss: {loss.item():.4f}, "
                      f"Loss1: {loss1.item():.4f}, Loss2: {loss2.item():.4f}")
                print(f"最近100步训练花费时间: {step_interval:.2f}秒")
        ...
        with torch.no_grad():
            ...
            if avg_loss < best_val_loss:
                ...
                torch.save(model_en.state_dict(), 'best_model_en.pkl')
                torch.save(model_de.state_dict(), 'best_model_de.pkl')
                torch.save(model_class.state_dict(), 'best_model_class.pkl')
                print("发现新最佳模型，已保存!")

            # 保存模型
            torch.save(model_en.state_dict(), f'model_en_{epoch + 1}.pkl')
            torch.save(model_de.state_dict(), f'model_de_{epoch + 1}.pkl')
            torch.save(model_class.state_dict(), f'model_class_{epoch + 1}.pkl')
...
model_en = EncoderNet([1, 1, 1, 1, 2, 1]).to(device)
model_de = DecoderNet([1, 1, 1, 1, 2, 1]).to(device)
...
# 加载预训练参数时设置strict=False，允许忽略新增层的参数
pretrained_en = torch.load('model_en.pkl',weights_only=True)
model_en.load_state_dict(pretrained_en, strict=False)  # 关键参数
model_en = model_en.to(device)

pretrained_de = torch.load('model_de.pkl',weights_only=True)
model_de.load_state_dict(pretrained_de, strict=False)
model_de = model_de.to(device)

pretrained_class = torch.load('model_class.pkl',weights_only=True)
model_class.load_state_dict(pretrained_class, strict=False)
...
#     {'params': model_en.en_layer6.parameters(), 'lr': args.lr},          # 新层高学习率
#     {'params': model_de.de_layer6.parameters(), 'lr': args.lr},          # 新层高学习率
#     {'params': model_class.parameters(), 'lr': args.lr},
# ])
# train(train_loader,val_loader,args.epochs,optimizer, criterion, criterion_clas)
#完整模型进行训练
optimizer = torch.optim.Adam([
    {'params': model_en.parameters(), 'lr': args.lr*0.5},
    {'params': model_de.parameters(), 'lr': args.lr*0.5},
    {'params': model_class.parameters(), 'lr': args.lr*0.5},
])
train(train_loader,val_loader,args.epochs,optimizer, criterion, criterion_clas)
# # 获取各模型参数（排除已单独指定的层）
# en_params = [p for name, p in model_en.named_parameters() if 'en_layer6' not in name]
# de_params = [p for name, p in model_de.named_parameters() if 'de_layer6' not in name]
#
# # 定义优化器（确保无参数重复）
# optimizer = torch.optim.Adam([
#     {'params': model_en.en_layer6.parameters(), 'lr': args.lr},          # 新层高学习率
#     {'params': model_de.de_layer6.parameters(), 'lr': args.lr},          # 新层高学习率
#     {'params': en_params, 'lr': args.lr * 0.1},                          # 旧层低学习率（排除en_layer6）
#     {'params': de_params, 'lr': args.lr * 0.1},                          # 旧层低学习率（排除de_layer6）
#     {'params': model_class.parameters(), 'lr': args.lr}                   # 分类层
# ])